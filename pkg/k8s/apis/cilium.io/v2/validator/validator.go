...
func NewNPValidator(logger *logrus.Logger) (*NPValidator, error) {
	...
	if err == nil {
		...
		if resCCNP.Spec != nil {
			if containsEmptyToFromEndpoint(resCCNP.Spec) {
				logger.Warn(warnEmptyToFromEndpointMessage)
				return nil
			}
		}
		...
		logger.Warnf("Error marshalling CCNP structure: %s", err)
	}
		...
		for _, r := range rule.Ingress {
			if len(r.FromEndpoints) > 0 {
				for _, epSel := range r.FromEndpoints {
					// Print a warning if the endpoint selector is a wildcard.
					if epSel.IsWildcard() {
						return true
					}
				}
