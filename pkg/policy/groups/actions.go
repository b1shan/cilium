...
// AddDerivativeCCNPIfNeeded will create a new CCNP if the given NetworkPolicy has any rule
// that need to create a new derivative policy.
// It returns a boolean, true in case that all actions are correct, false if
// something fails
...
func addDerivativePolicy(ctx context.Context, cnp *cilium_v2.CiliumNetworkPolicy, clusterScoped bool) error {
	...
	var derivativePolicy *cilium_v2.CiliumNetworkPolicy
	var derivativeErr, err error
	...
	// well.
	// If the createDerivative{CNP,CCNP}() fails, a new all block rule will be inserted and
